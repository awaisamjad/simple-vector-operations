#include <stdio.h>
#include <math.h>
#include <stdarg.h>
#include <stdlib.h>
#include <string.h>
#define PI 3.14159265358979323846
typedef struct
{
    float x;
    float y;
    float z;
} VEC3;
//~ VEC3 multiplied by scalar(float) and returned as VEC3
VEC3 scalar_mul_vec_3(float b, VEC3 a)
{
	VEC3 c;
	c.x = a.x * b;
	c.y = a.y * b;
	c.z = a.z * b;
	return c;
}

//~ VEC3 multiplied by VEC3 and returned as VEC3
VEC3 vec_mul_vec_3(VEC3 a, VEC3 b)
{
	VEC3 c;
	c.x = a.x * b.x;
	c.y = a.y * b.y;
	c.z = a.z * b.z;
	return c;
}

//~ return Magnitude of VEC3 as type float
float vec_mag_3(VEC3 a)
{
	return sqrt((a.x * a.x) + (a.y * a.y) + (a.z * a.z));
}

//~ return Normalised VEC3 as type VEC3
VEC3 vec_norm_3(VEC3 a)
{
	float norm_x = 0.0;
	float norm_y = 0.0;
	float norm_z = 0.0;
	float mag = vec_mag_3(a);

	if (mag == 0)
	{
		printf("Error: Cannot normalize a vector with magnitude 0\n");
		// Return a zero vector or another special value indicating an error
		return (VEC3){0.0, 0.0, 0.0};
	}
	else
	{
		norm_x = a.x / mag;
		norm_y = a.y / mag;
		norm_z = a.z / mag;
	}

	VEC3 norm = {norm_x, norm_y, norm_z};
	return norm;
}

//~ return Dot Product of VEC3 as type float
float dot_product_3(VEC3 a, VEC3 b)
{
	return a.x * b.x + a.y * b.y + a.z * b.z;
}

//~ return Cross Product of VEC3 as type VEC3
VEC3 cross_product_3(VEC3 a, VEC3 b)
{
	float i_comp = (a.y * b.z) - (a.z * b.y);
	float j_comp = (a.z * b.x) - (a.x * b.z); // Corrected calculation
	float k_comp = (a.x * b.y) - (a.y * b.x);
	VEC3 cross_product = {i_comp, j_comp, k_comp};
	return cross_product;
}

//~ sums n number of vectors and returns result as type VEC3
VEC3 add_vecs_3(int numberOfVectors, ...)
{
	va_list args;
	va_start(args, numberOfVectors);

	VEC3 result = {0.0, 0.0, 0.0};

	for (int i = 0; i < numberOfVectors; ++i)
	{
		VEC3 vec = va_arg(args, VEC3);
		result.x += vec.x;
		result.y += vec.y;
		result.z += vec.z;
	}

	va_end(args);

	return result;
}

//~ subtracts n number of vectors and returns result as type VEC3
VEC3 sub_vecs_3(int numberOfVectors, ...)
{
	va_list args;
	va_start(args, numberOfVectors);
	VEC3 result = va_arg(args, VEC3); //& gets the first value of the arguments instead of it being 0,0,0 as this wont be the actual subtraction

	for (int i = 1; i < numberOfVectors; ++i)
	{
		VEC3 vec = va_arg(args, VEC3);
		result.x -= vec.x;
		result.y -= vec.y;
		result.z -= vec.z;
	}

	va_end(args);

	return result;
}

//~ returns angle between two vectors as type float
float angle_between_vecs_3(VEC3 a, VEC3 b, char unit[7])
{
	float dot = dot_product_3(a, b);
	float mag_a = vec_mag_3(a);
	float mag_b = vec_mag_3(b);
	float angle;

	if (mag_a == 0 || mag_b == 0)
	{
		printf("Error: One or both vectors have zero magnitude.\n");
		return 0.0; // Return zero angle in case of an error
	}

	angle = acos(dot / (mag_a * mag_b));

	if (strcmp(unit, "radians") == 0)
	{
		// Angle is already in radians
		return angle;
	}
	else if (strcmp(unit, "degrees") == 0)
	{
		// Convert angle to degrees
		return angle * (180.0 / PI);
	}
	else
	{
		// Invalid unit specified
		printf("Error: Invalid unit specified. Please use 'radians' or 'degrees'.\n");
		return 0.0; // Return zero angle in case of an error
	}
}

//~ returns scalar triple product as type float
float scalar_triple_product_3(VEC3 a, VEC3 b, VEC3 c)
{
	float determinant = a.x * (b.y * c.z - b.z * c.y) - a.y * (b.x * c.z - b.z * c.x) + a.z * (b.x * c.y - b.y * c.x);
	return determinant;
}

//~ returns vector triple product as type VEC3
VEC3 vector_triple_product_3(VEC3 a, VEC3 b, VEC3 c)
{
	VEC3 result = cross_product_3(a, b);
	result = cross_product_3(result, c);
	return result;
}

//~ returns area of paralleogram formed by two vectors as type float
float area_of_parallelogram_formed_by_two_vecs_3(VEC3 a, VEC3 b)
{
	VEC3 cross = cross_product_3(a, b);
	return vec_mag_3(cross);
}

//~ returns area of triangle formed by two vectors as type float
float area_of_triangle_formed_by_two_vecs_3(VEC3 a, VEC3 b)
{
	return area_of_parallelogram_formed_by_two_vecs_3(a, b) / 2.0;
}